...
class MemoryManager:
    ...
    # _logical_memory: List[int] = None

    def __init__(self, strategy: Strategy):
        print("Inicializando o MemoryManager com a estratégia:", strategy.name)
        self._strategy = strategy
        self._physical_memory: List[str] = [None] * 128 # Criando a memória com 128 espaços
        # self._logical_memory: List[int] = [None] * 8 # Criando a memória virtual com 8 páginas de 16kb cada
    ...
    def write_using_paging(self, p: Process) -> None:
        ''' Method for write a process using paging  '''
        pass
        ...
        for i, block in enumerate(self._physical_memory):
            if block is None:
                ...
                if actual_size >= p.size_in_memory:
                    ...
                    self._insert_process_in_memory(
                        p, start, start + p.size_in_memory - 1)
                    return
            else:
    ...
    def write_using_best_fit(self, p: Process) -> None:
        ...
        if best_fit_start is not None:
            ...
            self._insert_process_in_memory(
                p, best_fit_start, best_fit_start + p.size_in_memory - 1)
        else:
    ...
    def write_using_worst_fit(self, p: Process) -> None:
        ...
        worst_fit_start = None  # Início do maior espaço encontrado
        worst_fit_size = 0  # Tamanho do maior espaço encontrado
        actual_size = 0  # Tamanho do espaço atual
        start = None  # Início do espaço atual
        ...
        for i, block in enumerate(extended_memory):
            # Se o bloco de memória for None
            if block is None:
                if start is None:
                    # Começam um novo espaço livre na memória
                    start = i
                    # print(f'Começa no bloco {start}, funcionando')
                actual_size += 1
                # print(f'ACTUAL SIZE: {actual_size}')
            # Se o bloco de memória não for None ou chegar ao fim da memória
            if block is not None or i == len(extended_memory) - 1:
                # Se o tamanho atual for maior que o maior tamanho guardado e maior que o tamanho do processo
                if actual_size > worst_fit_size and actual_size >= p.size_in_memory:
                    # print('ENTRANDO NO IF')
                    worst_fit_size = actual_size
        ...
        # Se o começo do worst fit não for None
        if worst_fit_start is not None:
            # Aloca o processo
            self._insert_process_in_memory(p, worst_fit_start, worst_fit_start + p.size_in_memory - 1)
            print(f"Processo {p.uid} alocado usando Worst Fit no espaço de tamanho {worst_fit_size}.")
        else:
    ...
    def _insert_process_in_memory(self, p: Process, start: int, end: int):
        ...
        if end < len(self._physical_memory):
            ...
            process_id = f"P{self._process_counter}"
            # Mapeia o UID para o ID acima
            self._process_map[p.uid] = process_id
            self._process_counter += 1
            for i in range(start, end + 1):
                # Usa o UID para rastrear qual processo ocupa o espaço
                self._physical_memory[i] = p.uid
        else:
